...
const SearchTabs = ({ activeTab, onTabChange }: SearchTabsProps) => {
  return (
    <div className="flex items-center justify-center gap-2 bg-white rounded-2xl p-2 shadow-sm border border-slate-100">
      {tabs.map((tab) => (
        <button
          key={tab.id}
          onClick={() => onTabChange(tab.id)}
          className={cn(
            "flex items-center flex-col sm:flex-row gap-2 px-6 py-3 rounded-xl font-medium transition-all duration-300",
            activeTab === tab.id
